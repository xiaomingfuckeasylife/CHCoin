...
public class DbVersion {
	...
	public static void init(){
		try{
			...
			int nextUpdate = -1;
			try{
				// if the version table is not exist will initialize the data base
	...
	private static void updateSql(Connection conn){
		...
		try(InputStream is = DbVersion.class.getClassLoader().getSystemResourceAsStream("1.sql");){
			InputStream tmpIs = is;
			if(is == null){
				try{
					tmpIs = new FileInputStream(System.getProperty("1.sql"));
				}catch(Exception ex){
					throw new RuntimeException("can not find 1.sql ");
				}
			}
